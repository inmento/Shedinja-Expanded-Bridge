...
| Gold or Silver with no Expanded Species | The bridge is inert; standalone Shedinja retains its normal Gen 2 behavior. |
| Gold or Silver with Expanded Species | The bridge preserves Shedinja’s framework-aware virtual/runtime identity and visible National Dex #292, restores its palette and menu icon, and fixes sparse OLD/National Pokédex ordering. |
...
| Expanded Species in Gold or Silver | [Expanded Species](https://github.com/mistermiracle3036/Expanded-Species) 0.6.5+. |

The bridge does **not** make either external framework mandatory. It does not load Gen 2 framework code in Red, Blue, or Yellow, and it does not load Crystal 251 code in Gold or Silver.

## Safety behavior

The Gold/Silver path refuses to use virtual index 292 if another active Expanded Species record already owns it. The Crystal path refuses to use index 252 if another active Gen 1 record already owns it. Those guards prevent two species from silently sharing a runtime identity.
